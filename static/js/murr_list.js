    ...
    let $name = 'murrList',
        methods = {
            ...
            bindEvent: function() {
                ...
                //     if (event.target.classList.contains('js-murr-card-title')) {
                //         $this[$name]('slideIn', $(this))
                //     }
                // });
                //
                // $('.murr-panel__close').click(function() {
                //     $this[$name]('slideOut')
                // });
            },
            slideOut: function() {
                let $this = $(this), $murrPanel = $('.murr-panel');
                $murrPanel.removeClass('murr-panel--is-visible');
            },
            slideIn: function($card) {
                let $this = $(this), data = $this.data($name),
                    $murrPanel = $('.murr-panel'), slug = $card.data('murr');

                $murrPanel.addClass('murr-panel--is-visible');
                $.ajax({
                    url: '/murrs/murr_detail/' + slug, data: data,
                    type: 'POST', dataType: 'json',
                    success: function(response) {
                        $('.murr-panel__content', $murrPanel).html(response.html);
                    }
                })
            },
            signUp: function() {
